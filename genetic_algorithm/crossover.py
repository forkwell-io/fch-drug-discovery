...
def cut_ring(mol):
    for i in range(10):
        ...
        try:
            ...
            # print("Exception during cut ring.")
            return None
...
def mol_OK(mol):
    try:
        ...
        if test_mol is None:
            # print("[DEBUG] Invalid Mol")
            return None
        else:
            # print("[INFO] Valid Mol found")
            return True
        # target_size = size_stdev * np.random.randn() + average_size  # parameters set in GA_mol
        # print("average size: {}".format(average_size))
...
def crossover_ring(parent_A, parent_B):
    ...
    if not parent_A.HasSubstructMatch(ring_smarts) and not parent_B.HasSubstructMatch(ring_smarts):
        # print("Both Parents have no SubtractMatch")
        return None
